...
| 3 | `doctors` | Profil & kredensial dokter | 10 | hospital-service |
...
| 17 | `pharmacy_staff` | Mapping pharmacist ke apotek | 2 | pharmacy-service |
| 18 | `pharmacy_inventory_adjustments` | Audit immutable perubahan stok manual | 0 | pharmacy-service |
| 19 | `referrals` | Rujukan antar rumah sakit | 6 | referral-service |
| 20 | `vital_signs` | Data tanda vital IoT (TimescaleDB) | 1.300 | iot-ingestion |
| 21 | `alerts` | Peringatan anomali tanda vital | 16 | alert-service |
| 22 | `notifications` | Notifikasi push/SMS/email | 27 | notification-service |
| 23 | `push_tokens` | Token push notification perangkat | 0 | notification-service |
| 24 | `refresh_tokens` | Token refresh autentikasi JWT | 0 | auth-service |
| 25 | `otp_codes` | Kode OTP verifikasi | 0 | auth-service |
| 26 | `auth_audit_logs` | Log audit login & autentikasi | 273 | auth-service |
| 27 | `medical_audit_logs` | Log audit akses rekam medis | 0 | patient-service |
| 28 | `iot_devices` | Perangkat IoT terdaftar | 3 | iot-ingestion |
| 29 | `patient_conditions` | Kondisi medis / diagnosis pasien | 37 | patient-service |
| 30 | `patient_allergies` | Alergi pasien | 17 | patient-service |
| 31 | `command_center_users` | Mapping user ke rumah sakit untuk Command Center | 0 | referral-service |
| 32 | `consent_records` | Rekam persetujuan privasi pengguna | 0 | auth-service |
| 33 | `schema_migrations` | Riwayat migrasi database | 10 | — |
...
| `status` | user_status | NOT NULL | `'PENDING_VERIFICATION'` | Enum: ACTIVE, INACTIVE, SUSPENDED, PENDING_VERIFICATION |
...
| `status` | consultation_status | NOT NULL | `'PENDING'` | Enum: PENDING, ACCEPTED, IN_PROGRESS, COMPLETED, CANCELLED, EXPIRED |
...
| `message_type` | message_type | NOT NULL | `'TEXT'` | Enum: TEXT, IMAGE, FILE, SYSTEM |
...
| `status` | prescription_status | NOT NULL | `'ISSUED'` | Enum: ISSUED, SENT_TO_PHARMACY, CONFIRMED, PREPARING, READY, DELIVERING, DELIVERED, CANCELLED |
...
**Deskripsi:** Mapping eksplisit user pharmacist ke apotek yang boleh dikelola.

**Jumlah Baris:** 2 (seed development/demo; production dikelola eksplisit)

| Kolom | Tipe | Nullable | Default | Keterangan |
|---|---|---|---|---|
| `user_id` | uuid | NOT NULL | — | FK → users.id (CASCADE), bagian dari primary key |
| `pharmacy_id` | uuid | NOT NULL | — | FK → pharmacies.id (CASCADE), bagian dari primary key |
| `is_active` | boolean | NOT NULL | `true` | Status assignment aktif |
| `created_at` | timestamptz | NOT NULL | `now()` | Waktu assignment |

---

### 18. `pharmacy_inventory_adjustments`

**Deskripsi:** Audit trail immutable untuk penyesuaian stok manual oleh user.
...
|---|---|---|---|---|
| `id` | uuid | NOT NULL | `gen_random_uuid()` | Primary key |
| `pharmacy_id` | uuid | NOT NULL | — | FK → pharmacies.id (RESTRICT) |
| `drug_id` | uuid | NOT NULL | — | FK → drugs.id (RESTRICT) |
| `actor_user_id` | uuid | NOT NULL | — | FK → users.id (RESTRICT) |
| `delta` | integer | NOT NULL | — | Perubahan stok, tidak boleh 0 |
| `reason` | text | NOT NULL | — | Alasan wajib diisi |
| `resulting_stock` | integer | NOT NULL | — | Stok setelah penyesuaian, minimal 0 |
| `created_at` | timestamptz | NOT NULL | `now()` | Waktu audit |
...
| `urgency_level` | varchar(20) | NOT NULL | `'NORMAL'` | Level: NORMAL, URGENT, EMERGENCY, CRITICAL |
...
| `activity_level` | varchar(20) | NULL | — | Nilai yang dipakai ingestion: resting, walking, running |
| `battery_level` | smallint | NULL | — | Level baterai perangkat (%) |
| `signal_strength` | smallint | NULL | — | Kekuatan sinyal (dBm) |
| `source` | vital_source | NOT NULL | `'WEARABLE'` | Enum: MANUAL, IOT_DEVICE, WEARABLE |
...
**Jumlah Baris:** 17 (satu baris per file migration yang telah diterapkan)
...
**Riwayat Migrasi (file yang diproses runner):**
| Versi | Keterangan |
|---|---|
| V001__create_extensions | Setup extensions PostgreSQL/TimescaleDB |
| V002__auth_schema | Schema autentikasi |
| V003__patient_schema | Schema pasien dan IoT |
| V004__consultation_schema | Schema konsultasi |
| V004z__geospatial_extensions | Extension geospatial |
| V005__prescription_pharmacy_schema | Schema resep dan apotek |
| V006__hospital_ambulance_referral_alert_schema | Schema rumah sakit, ambulans, referral, alert |
| V007__notifications_audit_consent_seed | Notifikasi, audit, consent, dan seed |
| V007z__legacy_pharmacy_aliases | Alias kompatibilitas legacy |
| V008__rich_seed_data | Seed data kaya |
| V009__data_enrichment | Pengayaan seed data |
| V010__data_enrichment_fix | Perbaikan seed data |
| V011__pharmacy_inventory_integrity | Integritas inventory apotek |
| V012__pharmacy_staff_assignment | Mapping pharmacist ke apotek |
| V013__inventory_adjustment_audit | Audit penyesuaian stok |
| V014__remove_legacy_pharmacy_aliases | Hapus alias legacy |
| V015__command_center_user_scope | Scope rumah sakit Command Center |
...
| `prescription_status` | ISSUED, SENT_TO_PHARMACY, CONFIRMED, PREPARING, READY, DELIVERING, DELIVERED, CANCELLED |
...
| `vital_source` | MANUAL, IOT_DEVICE, WEARABLE |
