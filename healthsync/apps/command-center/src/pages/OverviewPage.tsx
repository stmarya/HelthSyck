...
async function checkServiceHealth(url: string): Promise<boolean> {
  ...
  try {
    ...
    const dependenciesHealthy = dependencyFlags.every(
      ...
      {payload.map((p, i) => (
        ...
        >
          ...
          <span style={{ fontWeight: 700, color: "var(--color-text)" }}>
            {p.value}
            {p.name === "Heart Rate" ? " bpm" : p.name === "SpO₂" ? "%" : ""}
          </span>
        </div>
      ))}
    </div>
...
function StatCard({
  ...
  return (
    <div
      ...
      onMouseLeave={(e) =>
        onClick && ((e.currentTarget as HTMLDivElement).style.transform = "")
      }
    >
      <div
        style={{
...
async function fetchVitalsForChart(
  patientId: string,
  ...
  if (vitals.length === 0) {
    const latestRes = await patientClient.get(
      `/v1/patients/${patientId}/vitals/latest`,
    );
    const latest = (latestRes.data as { data: VitalRecord }).data;
    if (latest) {
      const t = new Date(latest.recorded_at).toLocaleTimeString("id-ID", {
        hour: "2-digit",
        minute: "2-digit",
      });
      return [
        {
          ...
          spo2: parseFloat(String(latest.spo2 ?? "0")),
        },
      ];
    }
    return [];
  ...
  return vitals.map((v) => ({
    time: new Date(v.recorded_at).toLocaleTimeString("id-ID", {
      hour: "2-digit",
      minute: "2-digit",
    }),
...
export default function OverviewPage() {
  ...
  const {
    ...
    dispatched: dispatchedAmbs,
  } = useAmbulances(15000);
  const {
    activeCount: activeConsultations,
    pendingCount: pendingConsultations,
    consultations,
  } = useConsultations(30000);
  ...
  const activeAmbs = ambulances.filter((a) =>
  ...
  const inTransitReferrals = referrals.filter(
  ...
  const availableBeds = hospitals.reduce(
    (s, h) => s + (h.available_beds ?? 0),
    0,
  );
  const totalIcu = hospitals.reduce((s, h) => s + (h.icu_total ?? 0), 0);
  const availableIcu = hospitals.reduce(
    (s, h) => s + (h.icu_available ?? 0),
    0,
  );
  const occupancyPct =
    totalBeds > 0
  ...
  const ambulanceBarData = [
    ...
    {
      name: "Kembali",
      value: ambulances.filter((a) => a.status === "RETURNING").length,
      color: "#3b82f6",
    },
  ...
  const referralUrgencyData = [
    {
      name: "Rutin",
      value: referrals.filter((r) => r.urgency_level === "ROUTINE").length,
      color: "#6b7280",
    },
    {
      name: "Mendesak",
      value: referrals.filter((r) => r.urgency_level === "URGENT").length,
      color: "#f59e0b",
    },
    {
      name: "Kritis",
      value: referrals.filter((r) => r.urgency_level === "CRITICAL").length,
      color: "#ef4444",
    },
    {
      name: "Darurat",
      value: referrals.filter((r) => r.urgency_level === "EMERGENCY").length,
      color: "#7c2d12",
    },
  ].filter((d) => d.value > 0);
    ...
    const checkHealth = () => {
      Promise.allSettled(
        SERVICE_LIST.map(([name, url]) =>
          checkServiceHealth(url).then(
            (healthy) => [name, healthy] as [string, boolean],
          ),
        ),
      )
        .then((results) => {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.statGrid} style={{ marginBottom: 20 }}>
        ...
        <StatCard
          ...
          color={
            healthyCount === totalServices && totalServices > 0
              ? "var(--color-success)"
              : "var(--color-warning)"
          }
          icon="💚"
      ...
      >
        ...
        <div
          ...
          onClick={() => navigate("/ambulance")}
        >
          <h2 className={styles.cardTitle} style={{ margin: "0 0 4px" }}>
            Status Armada Ambulans
          </h2>
          <p
            style={{
              fontSize: 12,
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          style={{ color: d.payload.color, fontWeight: 700 }}
                        >
                          {d.payload.name}:{" "}
                        </span>
                        <span
      ...
      >
        ...
        <div className={styles.card}>
          <h2 className={styles.cardTitle} style={{ margin: "0 0 16px" }}>
        ...
        <div className={styles.card}>
          ...
          >
            {[
              ...
              {
                label: "RS Mitra EMT",
                value: hospitals.filter((h) => h.is_emt_partner).length,
                color: "#7c3aed",
              },
